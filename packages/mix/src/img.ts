 ...
 */
export const getFileByUrl = (url: string, fileName?: string): Promise<File> => {
  return new Promise((resolve, reject) => {
 ...
 */
export const getExif = async (file: File): Promise<ExifReader.Tags> => {
  // 传入 File 对象或者 URL
...
// 传入URL，返回图片EXIF信息
export const getExifByURL = async (url: string): Promise<ExifReader.Tags> => {
  const tags = await ExifReader.load(url, { async: true })
  // MakerNote标签可以非常大。如果您正在解析大量文件并保存标记，请删除它以降低内存使用
  delete tags['MakerNote']
  // 如果想提取缩略图，可以这样使用
  // if (tags['Thumbnail'] && tags['Thumbnail'].image) {
...
/** EXIF图片文件，File + exifdata + iptcdata */
